# Data Engineering Portfolio

This repo is a collection of data projects focused on pipelines, reporting, automation, and practical analytics work.

Most of these projects come from real business problems — pulling data from APIs, cleaning it, storing it, and building dashboards on top of it.

## Projects

### 1. Amazon SP-API Sales Pipeline & Dashboard
Built a Python workflow that pulls Amazon sales and traffic data from SP-API, transforms it into a usable reporting format, loads it into Google Sheets, and feeds a Looker Studio dashboard.

**Tools used**
- Python
- Amazon SP-API
- Pandas
- Google Sheets API
- Looker Studio

**Project focus**
- API ingestion
- data cleaning
- reporting pipeline
- dashboard delivery

[Project link](ADD_LINK)

---

### 2. Sales Fact Backfill and Reporting Scripts
Python scripts for backfilling weekly sales data, standardizing fields, and preparing data for reporting.

**Tools used**
- Python
- Pandas
- APIs
- Google Sheets

**Project focus**
- backfills
- automation
- reporting consistency

[Project link](ADD_LINK)

---

### 3. SQL / analytics project
In progress.

## Notes
This portfolio is still being expanded as I clean up older projects and document them properly.
